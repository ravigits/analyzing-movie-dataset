...
def explore_data(dataset, start, end, rows_and_columns=False):
    ...
    if rows_and_columns:
        print(f"Dimensions : {array(dataset).shape}")
...
def duplicate_and_unique_movies(dataset, index_):
    ...
    for x in movienames:
        if x not in result:
            ...
            duplicates.append(x)
    print(f"Number of duplicate movies : {len(duplicates)}")
    print(f"Some duplicates : {duplicates}")
    return result
...
def rate_bucket(dataset, rate_low, rate_high=10):
    ...
    length = len(dataset)
    for i in range(length):
        if float(dataset[i][-4])>= rate_low and dataset[i][3]=='en':
...
mov_array = array(movies)
no_duplicates = duplicate_and_unique_movies(mov_array, -2)
...
for i in range(len(no_duplicates)):
    movies_clean.append(no_duplicates[i][-3:-1])
print(movies_clean)

# Calling movies_lang(), extract all the english movies and store it in movies_en.
movies_en = movies_lang(movies, 3, 'en')
explore_data(movies_en,0,5,False)


# Call the rate_bucket function to see the movies with rating higher than 8.
high_rated_movies = rate_bucket(dataset=movies,rate_low=8)
explore_data(high_rated_movies,0,5,False)
